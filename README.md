...
<style>
:root{
  --bg:#0b1220; --panel:#0f172a; --card:#111a2e; --ink:#e6eaf2; --muted:#9fb0c8;
  --line:#1f2a44; --teal:#00c4b3; --indigo:#5135ff; --ok:#10b981; --warn:#f59e0b; --bad:#ef4444;
  --radius:14px; --shadow:0 10px 30px #0008;
}
:root.light{
  --bg:#f7f9fc; --panel:#ffffff; --card:#ffffff; --ink:#0b1220; --muted:#5b6b81; --line:#e6ecf4;
  --shadow:0 6px 18px rgba(10,22,40,.08);
}
*{box-sizing:border-box} html,body{margin:0;height:100%;font:14px/1.45 Inter,system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:var(--ink);background:linear-gradient(180deg,var(--bg) 0%, #0e1628 100%)}
a{color:inherit;text-decoration:none}
button{font:600 14px Inter,system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif}
...
header{position:sticky;top:0;z-index:10;background:rgba(11,18,32,.65);backdrop-filter:saturate(120%) blur(10px);
  border-bottom:1px solid var(--line);padding:14px 20px}
:root.light header{background:rgba(255,255,255,.7)}

.nav{display:flex;gap:16px;align-items:center;max-width:1400px;margin:0 auto}
.logo{width:40px;height:40px;border-radius:50%;background:
  radial-gradient(120% 120% at 25% 20%, var(--teal), var(--indigo));
  box-shadow:0 0 0 2px #0b122055, var(--shadow)}
h1{margin:0;font-size:18px}
.nav-spacer{flex:1}
.btn{border:0;border-radius:12px;padding:9px 12px;cursor:pointer;transition:transform .05s ease}
.btn:active{transform:translateY(1px)}
.btn-ghost{background:transparent;color:var(--ink);border:1px solid var(--line)}
.btn-primary{background:linear-gradient(90deg,var(--teal),var(--indigo));color:#04101a}
.btn-danger{background:linear-gradient(90deg,#ff6b6b,#ff9b6b);color:#2a0b0b}
.switch{display:inline-flex;gap:8px;align-items:center}

main{max-width:1400px;margin:0 auto;padding:20px;display:grid;grid-template-columns:340px 1fr;gap:20px}
aside{background:var(--panel);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow)}
.section{padding:16px;border-bottom:1px solid var(--line)}
.section:last-child{border-bottom:0}
.section h2{margin:0 0 10px 0;font-size:16px}
.controls{display:grid;gap:12px}
input[type=file],input[type=text],input[type=number],select{
  width:100%;background:var(--card);color:var(--ink);border:1px solid var(--line);border-radius:10px;padding:9px 11px}
...
.kpi h3{margin:0 0 4px 0;font-size:12px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:.6px}
...
.toolbar{padding:10px 16px;border-bottom:1px solid var(--line);display:flex;gap:8px;align-items:center;flex-wrap:wrap}
...
th{position:sticky;top:0;background:var(--card);font-size:12px;color:var(--muted);z-index:1;cursor:pointer}
tr:hover td{background:rgba(81,53,255,.06)}
.badge{display:inline-block;padding:3px 9px;border-radius:999px;font-weight:700;font-size:12px;background:#243145;color:#bfe9ff}

.grid2x{display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:14px}

footer{padding:16px;border-top:1px solid var(--line);text-align:center;color:var(--muted)}
</style>
  ...
  <header>
    <div class="nav">
      ...
      <div>
        ...
        <small class="hint">Decision-grade opportunity scoring from marketplace + trend signals</small>
      </div>
      <div class="nav-spacer"></div>
      <div class="switch">
        <small class="hint">Theme</small>
        <button id="btnTheme" class="btn btn-ghost" title="Toggle Theme">Dark</button>
      </div>
      <button id="btnSave" class="btn btn-ghost" title="Save session to this browser">Save</button>
      <button id="btnLoad" class="btn btn-ghost" title="Load saved session">Load</button>
      <button id="btnClearAll" class="btn btn-danger" title="Clear all data">Clear</button>
    </div>
  </header>
    ...
    <aside>
      <div class="section">
        ...
        <div class="controls">
          ...
          <div class="grid2">
            <button id="btnSample" class="btn btn-ghost">Load sample data</button>
            <button id="btnReplace" class="btn btn-ghost" title="Replace current rows with new sample">Replace</button>
          </div>
      ...
      <div class="section">
        ...
        <div class="grid2">
          <div>
            <label>Demand</label>
            <input id="wDemand" type="range" min="0" max="60" step="1" value="35">
            <small id="wDemandVal" class="hint"></small>
          </div>
          <div>
            <label>Competition (1−HHI)</label>
            <input id="wComp" type="range" min="0" max="60" step="1" value="20">
            <small id="wCompVal" class="hint"></small>
          </div>
          <div>
            <label>Click Conv</label>
            <input id="wCC" type="range" min="0" max="60" step="1" value="15">
            <small id="wCCVal" class="hint"></small>
          </div>
          <div>
            <label>Price Posture</label>
            <input id="wPrice" type="range" min="0" max="60" step="1" value="15">
            <small id="wPriceVal" class="hint"></small>
          </div>
          <div>
            <label>Ad Feasibility</label>
            <input id="wAd" type="range" min="0" max="40" step="1" value="5">
            <small id="wAdVal" class="hint"></small>
          </div>
          <div>
            <label>Seasonality</label>
            <input id="wSeas" type="range" min="0" max="40" step="1" value="5">
            <small id="wSeasVal" class="hint"></small>
          </div>
          <div>
            <label>Social</label>
            <input id="wSoc" type="range" min="0" max="40" step="1" value="5">
            <small id="wSocVal" class="hint"></small>
          </div>
          <div>
            <label>&nbsp;</label>
            <div class="grid2">
              <button id="btnPresetBalanced" class="btn btn-ghost" title="Balanced weights">Balanced</button>
              <button id="btnPresetDemand" class="btn btn-ghost" title="Demand-heavy">Demand-heavy</button>
            </div>
            <div style="margin-top:8px">
              <button id="btnPresetRisk" class="btn btn-ghost" title="Risk-averse (competition/seasonality up)">Risk-averse</button>
            </div>
          </div>
        </div>
      ...
      <div class="section">
        ...
        <div class="row">
          <div>
            <label>Min OppScore</label>
            <input id="minOpp" type="number" value="0" min="0" max="100" step="1">
          </div>
          <div>
            <label>Search keyword</label>
            <input id="kwSearch" type="text" placeholder="filter…">
          </div>
        </div>
        <div class="row" style="margin-top:10px">
          <div>
            <label>Max HHI</label>
            <input id="maxHHI" type="number" value="0.65" min="0" max="1" step="0.01">
          </div>
          <div>
            <label>Min Seasonality</label>
            <input id="minSeas" type="number" value="0" min="0" max="1" step="0.01">
          </div>
        </div>
      </div>
      ...
      <div class="section">
        <h2>4) Export</h2>
        <div class="controls">
          <button id="btnExport" class="btn btn-primary">Export ranked CSV</button>
...
let trendsMap = {};     // keywordKey -> {series:[{t,y}], seasonality}
...
function applyTheme(t){ document.documentElement.className = (t==='light' ? 'light' : ''); els.btnTheme.textContent = (t==='light'?'Light':'Dark'); }
...
function setPresetBalanced(){
  els.wDemand.value=35; els.wComp.value=20; els.wCC.value=15; els.wPrice.value=15; els.wAd.value=5; els.wSeas.value=5; els.wSoc.value=5; setLabels(); renderAll();
}
function setPresetDemand(){
  els.wDemand.value=50; els.wComp.value=15; els.wCC.value=10; els.wPrice.value=10; els.wAd.value=5; els.wSeas.value=5; els.wSoc.value=5; setLabels(); renderAll();
}
function setPresetRisk(){
  els.wDemand.value=25; els.wComp.value=25; els.wCC.value=10; els.wPrice.value=10; els.wAd.value=5; els.wSeas.value=15; els.wSoc.value=10; setLabels(); renderAll();
}
...
function seasonality(series){
  ...
  const last4 = avg(vals.slice(-4)), prev4 = avg(vals.slice(-8,-4)), base = avg(vals.slice(0,4));
...
function weightNorm(w){
  const t = Object.values(w).reduce((a,b)=>a+b,0)||1;
  const out={}; for(const k in w) out[k]=w[k]/t; return out;
}
function priceScore(p, target, width){
  const d = Math.abs((p||0) - target);
  return clamp01(1 - Math.min(d/Math.max(1,width), 1));
}

function computeScores(){
  const s = getSettings(); const W = weightNorm(s.w);
  // normalize demand and CPC across rows
  ...
  rows.forEach(r=>{
    ...
    const social01 = 0.5; // placeholder for future input
...
function applyFiltersAndSort(){
  ...
  let out = rows.filter(r=>{
    return r.oppScore>=f.minOpp
        && r.hhi<=f.maxHHI
        && r.seasonality>=f.minSeas
...
function renderAll(){
  computeScores(); renderKPIs(); renderTable();
}
